...
function getMinMax(data) {
  ...
  console.log('test1', min, max);
  if ((min >= 1) && (min <= 3) && (max >= 1) && (max <= 3)) {
	min = Math.min(3.0, 1.0);
	max = Math.max(1.0, 3.0);
    return { min, max };
  }
  console.log('test2', min, max);

  min = Math.min(20.0, 5.0);
  max = Math.max(5.0, 20.0);
  return { min, max };
}
